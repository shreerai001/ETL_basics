...
fprintf(stderr,\"string_from_pyobj(str='%s',len=%d,inistr='%s',obj=%p)\\n\",
    ...
    if (obj == Py_None) {
        ...
        if (PyBytes_Check(obj)) {
            ...
            csv = PyUnicode_AsASCIIString(obj);
        }
        else {
            PyObject *tmp2;
            tmp2 = PyObject_Str(obj);
            if (tmp2) {
                csv = PyUnicode_AsASCIIString(tmp2);
                Py_DECREF(tmp2);
            }
            else {
                csv = NULL;
        ...
        if (csv == NULL) goto capi_fail;
        n = PyBytes_GET_SIZE(csv);
        buf = PyBytes_AS_STRING(csv);
    }
    if (*len == -1) {
...
int_from_pyobj(int* v, PyObject *obj, const char *errmess)
{
    ...
    csv = PyNumber_Long(obj);
    if (csv) {
        *v = Npy__PyLong_AsInt(csv);
        Py_DECREF(csv);
        return !(*v == -1 && PyErr_Occurred());
    }
    ...
    if (PyComplex_Check(obj)) {
        PyErr_Clear();
        csv = PyObject_GetAttrString(obj,\"real\");
    }
        ...
        PyErr_Clear();
        csv = PySequence_GetItem(obj, 0);
    }

    if (csv) {
        if (int_from_pyobj(v, csv, errmess)) {
            Py_DECREF(csv);
            return 1;
        }
        Py_DECREF(csv);
...
long_from_pyobj(long* v, PyObject *obj, const char *errmess) {
    ...
    if (csv) {
        if (long_from_pyobj(v, csv, errmess)) {
            Py_DECREF(csv);
            return 1;
        }
        Py_DECREF(csv);
...
long_long_from_pyobj(long_long* v, PyObject *obj, const char *errmess)
{
    ...
    if (csv) {
        if (long_long_from_pyobj(v, csv, errmess)) {
            Py_DECREF(csv);
            return 1;
        }
        Py_DECREF(csv);
...
double_from_pyobj(double* v, PyObject *obj, const char *errmess)
{
    ...
    csv = PyNumber_Float(obj);
    if (csv) {
        *v = PyFloat_AsDouble(csv);
        Py_DECREF(csv);
        return !(*v == -1.0 && PyErr_Occurred());
    }
    ...
    if (csv) {
        if (double_from_pyobj(v,csv,errmess)) {Py_DECREF(csv); return 1;}
        Py_DECREF(csv);
    }
    {
        PyObject* err = PyErr_Occurred();
...
complex_double_from_pyobj(complex_double* v, PyObject *obj, const char *errmess) {
    ...
    if (PySequence_Check(obj) && !(PyBytes_Check(obj) || PyUnicode_Check(obj))) {
        PyObject *csv = PySequence_GetItem(obj,0);
        if (csv) {
            if (complex_double_from_pyobj(v,csv,errmess)) {
                Py_DECREF(csv);
                return 1;
            }
            Py_DECREF(csv);
                  ...
                  const char *errmess)
{
    ...
    if (PyFunction_Check(fun)) {
        ...
        if (PyObject_HasAttrString(fun,\"im_func\")) {
            ...
            csv = PyObject_GetAttrString(fun,\"__call__\");
            if (PyObject_HasAttrString(csv,\"im_func\"))
                tmp_fun = PyObject_GetAttrString(csv,\"im_func\");
            else {
    ...
    if (PyObject_HasAttrString(tmp_fun,\"__code__\")) {
        if (PyObject_HasAttrString(csv = PyObject_GetAttrString(tmp_fun,\"__code__\"),\"co_argcount\")) {
            PyObject *tmp_argcount = PyObject_GetAttrString(csv,\"co_argcount\");
            Py_DECREF(csv);
    ...
    if (PyObject_HasAttrString(tmp_fun,\"__defaults__\")) {
        if (PyTuple_Check(csv = PyObject_GetAttrString(tmp_fun,\"__defaults__\")))
            opt = PyTuple_Size(csv);
        Py_XDECREF(csv);
    }
    ...
    *args = (PyTupleObject *)PyTuple_New(siz);
    ...
    if (xa != NULL)
        for (i=(*nofargs);i<siz;i++) {
            csv = PyTuple_GetItem((PyObject *)xa,i-(*nofargs));
            Py_INCREF(csv);
            PyTuple_SET_ITEM(*args,i,csv);
        }
