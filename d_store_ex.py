...
variables.set("SCRIPT_NAME", "d_store_ex")
...
try:
    ...
    execute_query(conn, "truncate bhatbhateni_stg.store")

    query = "copy into {0} from @{1}/store.gz FILE_FORMAT=(TYPE=csv field_delimiter='," \
            "' skip_header=0)" \
            "ON_ERROR ='ABORT_STATEMENT'".format("bhatbhateni_stg.store", "BHAT_BHATENI_STAGE")
    execute_query(conn, query)
